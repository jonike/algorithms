...
// Remove adjacent duplicates. "pred" dictates whether two adjacent characters should be treated as duplicates of each other.
bool duplicate_pred(const char lhs, const char rhs) {return lhs == rhs;}
inline void remove_adjacent(string & s){s.erase(unique(s.begin(), s.end(), duplicate_pred), s.end());}
